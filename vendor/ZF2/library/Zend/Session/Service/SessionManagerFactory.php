    ...
    public function createService(ServiceLocatorInterface $services) {
        ...
        if ($services->has('Zend\Session\Config\ConfigInterface')) {
            ...
            if (!$config instanceof ConfigInterface) {
                throw new ServiceNotCreatedException(sprintf(
                        'SessionManager requires that the %s service implement %s; received "%s"', 'Zend\Session\Config\ConfigInterface', 'Zend\Session\Config\ConfigInterface', (is_object($config) ? get_class($config) : gettype($config))
                ));
            }
        }
        ...
        if ($services->has('Zend\Session\Storage\StorageInterface')) {
            ...
            if (!$storage instanceof StorageInterface) {
                throw new ServiceNotCreatedException(sprintf(
                        'SessionManager requires that the %s service implement %s; received "%s"', 'Zend\Session\Storage\StorageInterface', 'Zend\Session\Storage\StorageInterface', (is_object($storage) ? get_class($storage) : gettype($storage))
                ));
            }
        }
        ...
        if ($services->has('Zend\Session\SaveHandler\SaveHandlerInterface')) {
            ...
            if (!$saveHandler instanceof SaveHandlerInterface) {
                throw new ServiceNotCreatedException(sprintf(
                        'SessionManager requires that the %s service implement %s; received "%s"', 'Zend\Session\SaveHandler\SaveHandlerInterface', 'Zend\Session\SaveHandler\SaveHandlerInterface', (is_object($saveHandler) ? get_class($saveHandler) : gettype($saveHandler))
                ));
            }
        }
